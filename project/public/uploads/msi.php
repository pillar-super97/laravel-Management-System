 ...
 *
 * This is an example Web Service which adds customers & order invoices to QuickBooks desktop
...
$map = array(
	...
	'*' => array( '_quickbooks_invoice_add_request', '_quickbooks_invoice_add_response' ),
...
{
    $Queue = new QuickBooks_WebConnector_Queue('mysqli://root:@localhost/superior-default_production');
	//$Queue->enqueue(QUICKBOOKS_ADD_CUSTOMER, $primary_key_of_your_customer);
	$Queue->enqueue(QUICKBOOKS_ADD_INVOICE, 5);
...
$dsn = 'mysqli://root:@localhost/superior-default_production';					// Connect to a MySQL database using the PHP MySQLi extension
$mysqli = new mysqli('localhost', 'root', '', 'superior-default_production');
...
{
  	$mysqli = new mysqli('localhost', 'root', '', 'superior-default_production');
		$sql = "SELECT * FROM invoices left join orders on invoices.order_id=orders.id WHERE invoices.id=1";
            ...
            <QBXML>
              <QBXMLMsgsRq onError="stopOnError">
                <InvoiceAddRq requestID="' . $requestID . '">
                  <InvoiceAdd>
                    <CustomerRef>
                      <FullName>Cristina Andres</FullName>
                    </CustomerRef>
                    <TxnDate>2015-01-23</TxnDate>
                    <RefNumber>TPY125</RefNumber>
                    <BillAddress>
                      <Addr1>himalaylok</Addr1>
                      <City>delhi</City>
                      <State>delhi</State>
                      <PostalCode>110074</PostalCode>
...
{
    $mysqli = new mysqli('localhost', 'root', '', 'superior-default_production');
    $results = $mysqli->query("UPDATE invoices SET qb_flag='0' WHERE id=1");
    //$fp =   fopen('file.log', 'a+');
    //fwrite($fp, array($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents));
    //fclose($fp);
...
////			}
////			$fp =   fopen('file.log', 'a+');
////                        fwrite($fp, implode("', '", array_values($arr)));
////                        fclose($fp);
//                        $results = $mysqli->query("UPDATE invoices SET qb_flag='0', qb_transaction_id='".mysqli_real_escape_string($arr['RefNumber'])."' WHERE id=1");
	...
	$xml = '<?xml version="1.0" encoding="utf-8"?>
		...
		<QBXML>
			<QBXMLMsgsRq onError="stopOnError">
				<CustomerAddRq requestID="' . $requestID . '">
					<CustomerAdd>
						<Name>kunal, LLC (' . mt_rand() . ')</Name>
						<CompanyName>pegasus, LLC</CompanyName>
 ...
 * @return void
 */
function _quickbooks_error_500($requestID, $user, $action, $ID, $extra, &$err, $xml, $errnum, $errmsg)
{
	mail('kunal.kumar@pegasusone.com',
		'QuickBooks error occured!',
		'QuickBooks thinks that ' . $action . ': ' . $ID . ' has a value which will not fit in a QuickBooks field...');
